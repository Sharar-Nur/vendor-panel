...
export default function AllItems(props) {
    ...
    // Get Item Lists
    useEffect(() => {
        PostLoginGetApi("shop/item", reqParam).then((responseJSON) => {
            ...
            <div className="custom-container-left float-left custom-scroll-auto min-w-[80%] w-full mb-20 pb-[100px] pr-3 text-justify" style={{ boxShadow: "3px 0 0px 0px rgb(0 0 0 / 1%)" }}>
                ...
                <div className="text-justify rounded-[5px] py-3">
                    ...
                    <div className="w-1/2 h-[68px] float-left text-right justify-end flex items-center">
                        <Link to={"/dashboard/store/add-item"} className="custom-btn mr-2 text-center">Create New Item</Link>
                ...
                <div className="w-full flex overflow-hidden text-left py-4 pt-2">
                    <div className="box hover-black-green w-[32%] py-5 h-auto mr-[1%] bg-[#FFFFFF] border-2 border-[#F5F5F5] text-[#222222] rounded-[5px] px-5">
                        ...
                        <h3 style={{ "fontSize": "22px" }} className="font-semibold  opacity-70">{apiData?.meta?.total}</h3>
                    </div>


                    <div className="box hover-black-green w-[32%] py-5 h-auto ml-[1%] mr-[1%] bg-[#FFFFFF] border-2 border-[#F5F5F5] text-[#222222] rounded-[5px] px-5" onClick={(e) => goToCategoryPage(e)}>
                        <Link to="#" className="block text-base font-medium">
                            Total Categories
                            <svg className="ml-1 display-revert" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.5 15L12.5 10L7.5 5" stroke="#222222" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    ...
                    <table className='min-w-full divide-y divide-gray-200 custom-table-hover '>
                        ...
                        <tbody className='divide-y divide-gray-200'>
                            ...
                            {itemList.map((data, index) => (
                                <tr key={index}>
                                    <td className='px-1 py-2 border-r-2 text-center' id='data'>
                                        <img src={data.image} title={data.name} alt={data.name} className='h-16 max-w-16 m-auto'></img>
                                    </td>
                                    <td className='px-6 py-2 border-r-2 '><Link to="/dashboard/store/items-details" className='cursor-pointer text-green-500' state={{ itemId: data.id }}>{data.name.replace(/^./, str => str.toUpperCase())}</Link></td>
                                    ...
                                    <td className='px-6 py-4 border-r-2 text-center'>
                                        {(data.is_publish === false) ?
                                            <>Local item</>
                                            :
                                            <>{(data.is_publish === true) ? <>Store item</> : <>Requested item</>}</>
                                        }
                                    </td>
                                            ...
                                            <div className='absolute -ml-20 -mt-2 border border-[1px solid rgb(245, 245, 245)] bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600'>
                                                <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconHorizontalButton">
                                                    <li className='border-b-[1px] border-b-slate-100'>
                                                        <Link to={`edit-item/${data.id}`} className="cursor-pointer block py-2 px-10 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                            Edit
                                                        </Link>
                                                    </li>
                                                    <li className='border-b-[1px] border-b-slate-100'>
                                                        <Link to={`duplicate-item/${data.id}`} className="cursor-pointer block py-2 px-10 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                            Duplicate
                                                        </Link>
                                                    </li>
                                                    <li>
