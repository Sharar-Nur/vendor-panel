...
const StoreEditItem = () => {
    ...
    useEffect(() => {
        ...
        try {
            PostLoginGetApi(`shop/item/${id}`).then((responseJSON) => {
    ...
    const onSubmit = (data) => {
        ...
        if (is_valid === true) {
            ...
            reqBody.append("category_id", previousData.category_id);
            ...
            reqBody.append("low_stock_alert", data.low_stock_alert);
            if (fileData !== undefined) {
                reqBody.append("image", fileData);
            }

            //  Update Item
            PostLoginPostApi(`shop/item/${id}`, reqBody, 1, 1).then((responseJSON) => {
                let response = JSON.stringify(responseJSON);
                response = JSON.parse(response);
    ...
    useEffect(() => {
        $("#low_stock_alert_div").addClass("hidden");
        if (previousData.low_stock_alert > 0) {
            $("#low_stock_alert_div").removeClass("hidden");
        }

    }, [previousData.low_stock_alert])
    ...
    const [totalVIndex, setTotalVIndex] = useState(0);
    useEffect(() => {

        let c = 0;
        if (previousData.addons) {
            previousData.addons.map((data) =>
                c += data.items.length
            )
            c++;
        }

        if (previousData.image !== undefined && previousData.image !== "") {
            setFile(previousData.image);
        }

        setTotalVIndex(c);

    }, [previousData.image, previousData.addons]);


    const handleUpdate = () => {

        PostLoginPostApi("shop/item/quantity/" + id, JSON.stringify({ stock_quantity: $("#quantity").val() }), 1, 0, 2).then((responseJSON) => {
            let response = JSON.stringify(responseJSON);
            response = JSON.parse(response);
    ...
    const handleUpdateLowStock = (is_message = 1) => {
        PostLoginPostApi("shop/item/low-stock/" + id, JSON.stringify({ low_stock_alert: $("#low_stock_alert").val() }), 1, 0, 2).then((responseJSON) => {
            ...
            if (is_message === 1) {
                ...
                if (response["status"] === 1) {
                    ...
                    if (responseData["code"] === 200) {
                        toast.success("Low stock quantity updated");
                        navigate("/dashboard/store");
                        return;
                    }
                ...
                <div className="font-medium min-h-[100vh] w-[100%] h-[100%] custom-container-left custom-scroll-auto">
                    ...
                    <form className="mb-10" action='' autoComplete="on" method='POST' encType='multipart/form-data' >
                        <header className="flex justify-between items-center p-4 border-b-2 border-dark-white mb-[60px]">
                            ...
                            <p className="-mr-24 text-xl text-black">Edit Item</p>
                            <div>
                        ...
                        <main className="max-w-2xl m-auto">
                            ...
                            <div className="mt-[5px] mb-[15px]">
                                ...
                                <div className="py-2 mt-1 relative">
                                    <select name="unit" {...register("unit")} id="unit" defaultValue={previousData.unit} className="w-full rounded border-solid border border-dark-white " required>
                            ...
                            <select {...register("storeCategory", { required: true })} className="w-full rounded border-solid border border-dark-white">
                                ...
                                <div className="relative pb-1 pt-1 mb-1">
                                    <input type="text" name="unitPrice" {...register("unitPrice")} defaultValue={previousData.unit_price} id="unitPrice" placeholder=" " className="pl-14 w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />

                                    <label htmlFor="unitPrice" className="absolute pl-14 text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1">Unit price</label>
                                ...
                                {/* <div className="relative pb-1 pt-1 mb-1">
                                    <select name="itemCategory" id="itemCategory" {...register("itemCategory")} defaultValue={previousData.category.id} className='w-full rounded border-solid border border-dark-white' required>
                            ...
                            <div className="mt-[5px] mb-[15px]">
                                <div className="relative pb-1 pt-1 mb-1">
                                    <input type="number" name="barcodeNumber" {...register("barcodeNumber")} defaultValue={previousData.barcode} id="barcodeNumber" placeholder=" " className="w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
                                ...
                                {file &&
                                    <div className='flex justify-center my-5'>
                                        <img src={file} alt="store item preview" className='h-[100px] ' />
                                    </div>
                                }
                            ...
                            <div className='block border border-[#e5e7eb] rounded-md py-5 px-4 mt-2'>
                                ...
                                <div className="relative mt-2 pb-1 pt-1 mb-1">
                                    <input type="text" name="quantity" {...register("quantity")} defaultValue={previousData.stock_quantity} id="quantity" placeholder=" " className="w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />

                                    <label htmlFor="quantity" className="absolute text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1">Update Quantity</label>
                                </div>
                                <button type="Button" onClick={handleUpdate} className="w-full text-white bg-[#1A202C] mt-3 py-[12px] px-[32px] h-[48px] rounded-[3px] cursor-pointer flex justify-center mb-3">
                                    Update
                                </button>
                            </div>
                            ...
                            <div className='block border border-[#e5e7eb] rounded-md py-5 px-4 mt-2 mb-5'>
                                <label className="toggle my-1 w-full">
                                    ...
                                    <input id="apply_item_or_category" name="apply_item_or_category" {...register("apply_item_or_category")} onChange={(event) => lowStockSwitch(event)} defaultChecked={previousData.low_stock_alert > 0 ? true : false} className="toggle-checkbox" value="1" type="checkbox" />
                                    <div className="toggle-switch float-right"></div>
                                    <span className="toggle-label text-base font-semibold">Low Stock Alert</span>
                                </label>

                                <div id='low_stock_alert_div'>
                                    <div className="relative mt-2 pb-1 pt-1 mb-1">
                                        <input type="text" name="low_stock_alert" {...register("low_stock_alert")} defaultValue={previousData.low_stock_alert} id="low_stock_alert" placeholder=" " className="w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
                                    ...
                                    <button type="Button" onClick={() => handleUpdateLowStock(1)} className="w-full  text-white bg-[#1A202C] mt-3 py-[12px] px-[32px] h-[48px] rounded-[3px] cursor-pointer flex justify-center mb-3">
                                        Update
                                    </button>
                                </div>
                            ...
                            <input type="hidden" id="vIndex" value={totalVIndex} />
                            ...
                            <div className='block border border-[#e5e7eb] rounded-md py-5 px-4 mt-2 pt-3 w-full'>
                                ...
                                {(previousData.addons) !== null ?
                                    <>
                                        <div id="variant-box0" className='variant-box'>
                                            <div className='w-full flex'>

                                                <div className="relative pb-1 pt-2 w-3/5">
                                                    <input type="text" name="variant_name" id="variant_name0" defaultValue={previousData.addons[0].name} placeholder=" " className="w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
                                                    <label htmlFor="variant_name0" className="absolute -mt-1 text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-2 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:mt-1 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1" >Variant Name</label>
                                                </div>
                                                ...
                                                <div className="relative pl-2 pb-1 mb-1 w-2/5">
                                                    <div className="py-2 relative">
                                                        <div id="mm-dropdown0" className="mm-dropdown cursor-pointer" onClick={() => showDropDown(0)}>
                                                            {(previousData.addons[0].is_radio_button === "1") ?
                                                ...
                                                {(previousData.addons[0].is_radio_button === "1") ?
                                                    <>
                                                        <div id="required0" className='bg-[#E5F9EA] text-[#009E22] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                            Required
                                                        </div>
                                                    ...
                                                    <>
                                                        ...
                                                        <div id="optional0" className='bg-[#F5F5F5] text-[#999999] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                            Optional
                                                        </div>
                                                    </>
                                                }
                                            ...
                                            <div className='variant-option-fixed-box'>
                                                {previousData.addons[0].items.map((data, index) =>
                                                    <div key={index} id={`variant_option_fixed_0${index}`} className="variant-option-fixed flex items-center mt-1 py-1">
                                                        {(previousData.addons[0].is_radio_button === "1") ?
                                                            <>
                                                                <img src="data:image/png;base64," alt="Variant Option" className="vo_radio_icon h-[18px] " />
                                                                <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAABISURBVHgB7dSxDQAgCETRwzgICfuPRGQTlYIJvFjxG61eoEHcfQFQcIpBxDKd9TMzwUN3053vALkGG2ywwT+g1B1jlRMGeMUBLKAONRePu7wAAAAASUVORK5CYII=" alt="Variant Option" className="vo_checkbox_icon h-[18px] hidden" />
                                                            </>
                                                            :
                                                            <>
                                                                <img src="data:image/png;base64," alt="Variant Option" className="vo_radio_icon h-[18px] hidden" />
                                                        ...
                                                        <div className="flex justify-between w-full pl-2">
                                                            <div className="w-[70%] pl-0 pt-0.5 addons-item-name">{data?.name}</div>
                                                            <div className="ml-2 w-[23%] pl-0 pt-1 text-center">
                                                                + ৳
                                                                <span className="addons-item-price" style={{ marginLeft: "7px" }}>{data?.unit_price}</span>
                                                            </div>
                                                            <div className="h-6 w-6 flex items-center mt-1 cursor-pointer" onClick={() => removeCurrentVariantItem(`0${index}`)}>
                                                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.4099 12L19.7099 5.71C19.8982 5.5217 20.004 5.2663 20.004 5C20.004 4.7337 19.8982 4.47831 19.7099 4.29C19.5216 4.1017 19.2662 3.99591 18.9999 3.99591C18.7336 3.99591 18.4782 4.1017 18.2899 4.29L11.9999 10.59L5.70994 4.29C5.52164 4.1017 5.26624 3.99591 4.99994 3.99591C4.73364 3.99591 4.47824 4.1017 4.28994 4.29C4.10164 4.47831 3.99585 4.7337 3.99585 5C3.99585 5.2663 4.10164 5.5217 4.28994 5.71L10.5899 12L4.28994 18.29C4.19621 18.383 4.12182 18.4936 4.07105 18.6154C4.02028 18.7373 3.99414 18.868 3.99414 19C3.99414 19.132 4.02028 19.2627 4.07105 19.3846C4.12182 19.5064 4.19621 19.617 4.28994 19.71C4.3829 19.8037 4.4935 19.8781 4.61536 19.9289C4.73722 19.9797 4.86793 20.0058 4.99994 20.0058C5.13195 20.0058 5.26266 19.9797 5.38452 19.9289C5.50638 19.8781 5.61698 19.8037 5.70994 19.71L11.9999 13.41L18.2899 19.71C18.3829 19.8037 18.4935 19.8781 18.6154 19.9289C18.7372 19.9797 18.8679 20.0058 18.9999 20.0058C19.132 20.0058 19.2627 19.9797 19.3845 19.9289C19.5064 19.8781 19.617 19.8037 19.7099 19.71C19.8037 19.617 19.8781 19.5064 19.9288 19.3846C19.9796 19.2627 20.0057 19.132 20.0057 19C20.0057 18.868 19.9796 18.7373 19.9288 18.6154C19.8781 18.4936 19.8037 18.383 19.7099 18.29L13.4099 12Z" fill="#999999"></path></svg>
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                                            ...
                                            <div className='variant-option flex items-center mt-2'>
                                                {(previousData.addons[0].is_radio_button === "1") ?
                                                    <>
                                                        <img src={RadioIcon2} alt="Variant Option" className='vo_radio_icon h-[18px]'></img>
                                                    ...
                                                    <>
                                                        ...
                                                        <img src={CheckBoxIcon2} alt="Variant Option" className='vo_checkbox_icon h-[18px]'></img>
                                                    </>
                                                }
                                                ...
                                                <div className='flex justify-between w-full pl-2'>
                                                    ...
                                                    <input type="text" name="add-price" onKeyDown={onEnterPressPrice} className='ml-2 w-[27%] text-center add-price border-transparent focus:border-green-700 focus:ring-0' placeholder='+ Add Price' />
                                                </div>
                                            </div>
                                        </div>

                                        <input type="hidden" value={previousData.addons.length} id="variant_id" />
                                        <div id="more_variant" style={{ marginTop: "18px" }}>
                                            {previousData.addons.map((data2, index2) =>
                                                (index2 > 0) &&
                                                <div key={index2}>
                                                    <div id={`variant-box${index2}`} className='variant-box' style={{ borderTop: "2px dotted rgb(221, 221, 221)", marginTop: "6px", paddingTop: "20px", marginBottom: "20px" }}>

                                                        <div className="text-[#FD171B] flex float-right items-center font-semibold cursor-pointer close-btn" onClick={() => removeVariant(`${index2}`)}><svg width="8" height="8" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.82067 4.99913L9.49567 1.32996C9.60552 1.22012 9.66723 1.07114 9.66723 0.915798C9.66723 0.760456 9.60552 0.611475 9.49567 0.501632C9.38583 0.391788 9.23685 0.330078 9.08151 0.330078C8.92616 0.330078 8.77718 0.391788 8.66734 0.501632L4.99817 4.17663L1.32901 0.501632C1.21916 0.391788 1.07018 0.330078 0.914841 0.330078C0.759498 0.330078 0.610518 0.391788 0.500674 0.501632C0.39083 0.611475 0.329121 0.760456 0.329121 0.915798C0.329121 1.07114 0.39083 1.22012 0.500674 1.32996L4.17567 4.99913L0.500674 8.6683C0.445999 8.72253 0.402603 8.78704 0.372988 8.85813C0.343372 8.92921 0.328125 9.00546 0.328125 9.08246C0.328125 9.15947 0.343372 9.23572 0.372988 9.3068C0.402603 9.37789 0.445999 9.4424 0.500674 9.49663C0.554902 9.55131 0.61942 9.5947 0.690504 9.62432C0.761589 9.65393 0.837834 9.66918 0.914841 9.66918C0.991848 9.66918 1.06809 9.65393 1.13918 9.62432C1.21026 9.5947 1.27478 9.55131 1.32901 9.49663L4.99817 5.82163L8.66734 9.49663C8.72157 9.55131 8.78609 9.5947 8.85717 9.62432C8.92826 9.65393 9.0045 9.66918 9.08151 9.66918C9.15851 9.66918 9.23476 9.65393 9.30584 9.62432C9.37693 9.5947 9.44144 9.55131 9.49567 9.49663C9.55035 9.4424 9.59375 9.37789 9.62336 9.3068C9.65298 9.23572 9.66822 9.15947 9.66822 9.08246C9.66822 9.00546 9.65298 8.92921 9.62336 8.85813C9.59375 8.78704 9.55035 8.72253 9.49567 8.6683L5.82067 4.99913Z" fill="#FD171B"></path></svg><span className="ml-1 text-[12px] float-right">close</span></div>

                                                        <div className='w-full flex'>
                                                            <input type="hidden" name="variant_box_index" value={index2} />
                                                            <div className="relative pb-1 pt-2 w-3/5">
                                                                <input type="text" name="variant_name" id={`variant_name${index2}`} defaultValue={data2.name} placeholder=" " className="w-full rounded border-solid border border-dark-white placeholder:text-ash appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
                                                                <label htmlFor={`variant_name${index2}`} className="absolute -mt-1 text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-2 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:mt-1 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1" >Variant Name</label>
                                                            </div>


                                                            <div className="relative pl-2 pb-1 mb-1 w-2/5">
                                                                <div className="py-2 relative">
                                                                    <div id={`mm-dropdown${index2}`} className="mm-dropdown cursor-pointer" onClick={() => showDropDown(index2)}>
                                                                        {(previousData.addons[index2].is_radio_button === "1") ?
                                                                            <div className="textfirst">
                                                                                <img src={RadioIcon} alt="Radio" width="20" height="20" /> Single Choice
                                                                            </div>
                                                                            :
                                                                        ...
                                                                        <input type="hidden" className={`option${index2}`} name="is_radio_button" value="" />
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            {(previousData.addons[index2].is_radio_button === "1") ?
                                                                <>
                                                                    <div id={`required${index2}`} className='bg-[#E5F9EA] text-[#009E22] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                                        Required
                                                                    </div>
                                                                    <div id={`optional${index2}`} className='bg-[#F5F5F5] hidden text-[#999999] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                                        Optional
                                                                    </div>
                                                                </>
                                                                :
                                                                <>
                                                                    <div id={`required${index2}`} className='bg-[#E5F9EA] hidden text-[#009E22] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                                        Required
                                                                    </div>
                                                                    <div id={`optional${index2}`} className='bg-[#F5F5F5] text-[#999999] ml-1 -mr-1 px-2 py-1 h-6 text-[10px] mt-4 font-medium rounded-3xl'>
                                                                        Optional
                                                                    </div>
                                                                </>
                                                            }
                                                        ...
                                                        <div className='variant-option-fixed-box'>
                                                            {previousData.addons[index2].items.map((data3, index3) =>
                                                                <div key={index3} id={`variant_option_fixed_${index2}${index3}`} className="variant-option-fixed flex items-center mt-1 py-1">
                                                                    {(previousData.addons[index2].is_radio_button === "1") ?
                                                                        <>
                                                                            <img src="data:image/png;base64," alt="Variant Option" className="vo_radio_icon h-[18px] " />
                                                                            <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAABISURBVHgB7dSxDQAgCETRwzgICfuPRGQTlYIJvFjxG61eoEHcfQFQcIpBxDKd9TMzwUN3053vALkGG2ywwT+g1B1jlRMGeMUBLKAONRePu7wAAAAASUVORK5CYII=" alt="Variant Option" className="vo_checkbox_icon h-[18px] hidden" />
                                                                    ...
                                                                    <div className="flex justify-between w-full pl-2">
                                                                        <div className="w-[70%] pl-0 pt-0.5 addons-item-name">{data3?.name}</div>
                                                                        <div className="ml-2 w-[23%] pl-0 pt-1 text-center">
                                                                            + ৳
                                                                            <span className="addons-item-price" style={{ marginLeft: "7px" }}>{data3?.unit_price}</span>
                                                                        </div>
                                                                        <div className="h-6 w-6 flex items-center mt-1 cursor-pointer" onClick={() => removeCurrentVariantItem(`${index2}${index3}`)}>
                                                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.4099 12L19.7099 5.71C19.8982 5.5217 20.004 5.2663 20.004 5C20.004 4.7337 19.8982 4.47831 19.7099 4.29C19.5216 4.1017 19.2662 3.99591 18.9999 3.99591C18.7336 3.99591 18.4782 4.1017 18.2899 4.29L11.9999 10.59L5.70994 4.29C5.52164 4.1017 5.26624 3.99591 4.99994 3.99591C4.73364 3.99591 4.47824 4.1017 4.28994 4.29C4.10164 4.47831 3.99585 4.7337 3.99585 5C3.99585 5.2663 4.10164 5.5217 4.28994 5.71L10.5899 12L4.28994 18.29C4.19621 18.383 4.12182 18.4936 4.07105 18.6154C4.02028 18.7373 3.99414 18.868 3.99414 19C3.99414 19.132 4.02028 19.2627 4.07105 19.3846C4.12182 19.5064 4.19621 19.617 4.28994 19.71C4.3829 19.8037 4.4935 19.8781 4.61536 19.9289C4.73722 19.9797 4.86793 20.0058 4.99994 20.0058C5.13195 20.0058 5.26266 19.9797 5.38452 19.9289C5.50638 19.8781 5.61698 19.8037 5.70994 19.71L11.9999 13.41L18.2899 19.71C18.3829 19.8037 18.4935 19.8781 18.6154 19.9289C18.7372 19.9797 18.8679 20.0058 18.9999 20.0058C19.132 20.0058 19.2627 19.9797 19.3845 19.9289C19.5064 19.8781 19.617 19.8037 19.7099 19.71C19.8037 19.617 19.8781 19.5064 19.9288 19.3846C19.9796 19.2627 20.0057 19.132 20.0057 19C20.0057 18.868 19.9796 18.7373 19.9288 18.6154C19.8781 18.4936 19.8037 18.383 19.7099 18.29L13.4099 12Z" fill="#999999"></path></svg>
                                                                        </div>
                                                                    </div>
                                                                </div>
