...
const AllConnects = (props = '') => {
    ...
    useEffect(() => {
        ...
        // Get All Connects
        PostLoginGetApi("shop/connect", reqParam).then((responseJSON) => {
                        ...
                        <div className="border rounded-lg shadow overflow-hidden">
                            <table className="min-w-full divide-y divide-gray-200 custom-table-hover">
                                ...
                                <tbody className='divide-y divide-gray-200'>
                                    ...
                                    {
                                        connectList.map((data, index) => (
                                            (data?.user !== null) &&
                                            <tr key={index}>
                                                <td className='px-6 py-4 border-r-2'><Link to={`/dashboard/connects/connect-details/${data.id}`} className="cursor-pointer text-green-500">{data.user?.name}</Link></td>
