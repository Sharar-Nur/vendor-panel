...
const BusinessStep5 = () => {
    ...
    useEffect(()=>{
        if (state === null || state.steps === undefined || state.steps === "" || state.steps === null || state.uploadDynamicDoc === undefined || state.uploadDynamicDoc === "" || state.uploadDynamicDoc === null) {
            toast.error("Please fill up business details first.");
            return navigate("/dashboard/business-details");
        }

        if(businessLogo.length > 0) {
            navigate("/dashboard/business-details-upload-logo-preview-and-submit", {
                state: {
    ...
    },[state, businessLogo, navigate, uploadFile]);
    ...
    function handleFile(files) {
        ...
        if( files[index]["type"] !== "image/png" && files[index]["type"] !== "image/jpg" && files[index]["type"] !== "image/jpeg" && files[index]["type"] !== "image/svg" ) {
            toast.error("Invalid File Type. Please upload valid image File.");
            return;
        }
    ...
    return (
        <div className="font-medium min-h-[100vh] w-[100%] h-[100%]">
            <header className="flex justify-between items-center p-4 border-b-2 border-dark-white mb-[60px]">
                ...
                <div className="mr-3">
                    Step <strong>{parseInt(state.steps)+1}</strong> out of <strong>{parseInt(state.steps)+2}</strong>
                </div>
            </header>
